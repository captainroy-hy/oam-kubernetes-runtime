...
func (v *ValidatingAppConfig) PrepareForValidation(ctx context.Context, c client.Reader, dm discoverymapper.DiscoveryMapper, ac *v1alpha2.ApplicationConfiguration) error {
	v.appConfig = *ac
	v.validatingComps = make([]ValidatingComponent, len(ac.Spec.Components))
	for _, acc := range ac.Spec.Components {
		...
		if err := json.Unmarshal(comp.Spec.Workload.Raw, &wl); err != nil {
		...
		tmp.validatingTraits = make([]ValidatingTrait, len(acc.Traits))
		for _, t := range acc.Traits {
			...
			if err := json.Unmarshal(t.Trait.Raw, &tContent); err != nil {
